...
module Diagrams.SVG.Fonts.Text
...
import Diagrams.SVG.Fonts.Fonts (lin)
import Diagrams.SVG.Fonts.ReadFont
import Diagrams.SVG.Fonts.CharReference (characterStrings)
...
textSVG' topts text =
  ...
  where
    ...
    polygonChar (ch,a) = (fromMaybe mempty (H.lookup ch outl)) <> (underlineChar a)
    ...
    ligatures = ((filter ((>1) . length)) . H.keys . fontDataGlyphs) fontD
